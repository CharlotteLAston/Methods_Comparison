...
name<- paste(study,"all_length",sep="_")

factor.vars=c("method")# Status as a Factor with two levels
out.all=list()
var.imp=list()

# Loop through the FSS function for each Taxa----
  use.dat <- as.data.frame(dat) %>% 
  filter(scientific %in% unique.vars.use)
  use.dat$method <- as.factor(use.dat$method)
  use.dat$sample <- as.factor(use.dat$sample)
  Model1=gam(length~s(depth,k=3,bs='cr')+s(sample,bs='re'),
