...
setwd(out_dir)
#read in data 
dat1 <- read.csv("Ningaloo_PtCloates_BOSS-BRUV_greater_less_all.var.imp.csv")%>% #from local copy
  rename(resp.var=X)%>%
...
dat2 <- read.csv("Ningaloo_PtCloates_BOSS-BRUV_greater_less_by_species_all.var.imp.csv")%>% #from local copy
  rename(resp.var=X)%>%
  gather(key=predictor,value=importance,2:ncol(.))%>%
  mutate(importance = ifelse(importance > 1, 1, importance)) %>% 
  glimpse()
...
dat.greater.less <- greater_less %>%
  ...
  mutate(resp.var=factor(resp.var, levels = c("< Length Maturity","> Length Maturity",	
                                              ...
                                              "> Length Maturity_Lethrinidae Lethrinus nebulosus",
                                              "< Length Maturity_Lutjanidae Pristipomoides multidens",
                                              "> Length Maturity_Lutjanidae Pristipomoides multidens")))%>%
  # greater or less than length maturity
  mutate(label=ifelse(predictor=="method","X",label))%>%
  mutate(label=ifelse(predictor=="depth"&resp.var=="> Length Maturity","X",label))%>%
  mutate(label=ifelse(predictor=="relief"&resp.var=="> Length Maturity","X",label))%>%
  mutate(label=ifelse(predictor=="depth"&resp.var=="< Length Maturity","X",label))%>%
  mutate(label=ifelse(predictor=="roughness"&resp.var=="< Length Maturity","X",label))%>%
  # greater or less than length maturity by species
  mutate(label=ifelse(predictor=="sdrel"&resp.var=="> Length Maturity_Lethrinidae Lethrinus miniatus","X",label))%>%
  ...
  mutate(label=ifelse(predictor=="depth"&resp.var=="> Length Maturity_Lethrinidae Lethrinus nebulosus","X",label))%>%
  mutate(label=ifelse(predictor=="relief"&resp.var=="> Length Maturity_Lethrinidae Lethrinus nebulosus","X",label))%>%
  mutate(label=ifelse(predictor=="method"&resp.var=="> Length Maturity_Lethrinidae Lethrinus nebulosus","X",label))%>%
  
  mutate(label=ifelse(predictor=="depth"&resp.var=="< Length Maturity_Lutjanidae Pristipomoides multidens","X",label))%>%
  mutate(label=ifelse(predictor=="roughness"&resp.var=="< Length Maturity_Lutjanidae Pristipomoides multidens","X",label))%>%
  mutate(label=ifelse(predictor=="method"&resp.var=="< Length Maturity_Lutjanidae Pristipomoides multidens","X",label))%>%
...
imp.species.greater.less <- ggplot(dat.greater.less%>%dplyr::filter(resp.var%in%c("> Length Maturity_Lethrinidae Lethrinus miniatus",
                                                                               "> Length Maturity_Lethrinidae Lethrinus nebulosus",
                                                                               "> Length Maturity_Lutjanidae Pristipomoides multidens",
                                                                               "< Length Maturity_Lutjanidae Pristipomoides multidens")), 
                   aes(x=predictor,y=resp.var,fill=importance)) +
...
setwd(fig_dir)
save_plot("ningaloo.importance.greater.less.png", gg.importance.greater.less, base_height = 5, base_width = 6.275)
